...
const Header = () => {
  ...
  return (
    <>
      <Sidebar isOpen={isSidebarOpen} toggleSidebar={toggleSidebar} />
      <header
        className="fixed top-0 left-0 w-full p-0 shadow-md transition-all duration-800"
        style={{
          backgroundColor: "#EEF2FF",
          borderRadius: "20px", // Curved corners applied here
        }}
      >
        <div className="flex items-center justify-between max-w-10xl mx-auto">
          ...
          {/* Profile Section */}
          <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }} className="flex items-center space-x-4">
            {loading ? (
              <span>Loading...</span>
            ) : (
              <span className="text-lg font-bold text-blue-600">
            ...
            <button
              onClick={handleLogoutConfirmation}
              className="flex items-center gap-1 px-2 py-2  text-white transition bg-red-500 rounded-lg pl-1 pr-1 shadow- hover:bg-red-600"
            >
              <LogOut size={18} />
            </button>
          </motion.div>
